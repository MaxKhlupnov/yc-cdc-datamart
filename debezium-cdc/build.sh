...
# create trusted CA store and add YandexCA.crt as trusted root
docker build -t debezium/kafka:1.6 -f ./kafka/Dockerfile kafka/.
docker build -t debezium/connect-base:1.6 -f ./connect-base/Dockerfile connect-base/.
docker build -t debezium/connect:1.6 -f ./connect/Dockerfile connect/.
